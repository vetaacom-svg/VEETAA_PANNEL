...
export const getDispatchRecommendation = async (
  ...
  const driverData = availableDrivers.map(d => ({
    id: d.id,
    name: d.name,
    distanceToStore: calculateDistance(d.lat, d.lng, Number(store.latitude), Number(store.longitude)),
    distanceFromStoreToUser: calculateDistance(Number(store.latitude), Number(store.longitude), user.lat, user.lng)
  }));
  ...
  const prompt = `
    ...
    - Target Store: ${store.name} at [${store.latitude}, ${store.longitude}]
